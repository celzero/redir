...
export async function registerDevice(env, req) {
  ...
  if (cachedDidSig === signotok) {
    // TODO: const meta = await consumejson(req);
...
/**
 * Verifies that the cid was generated by us (via the HMAC flow or legacy DB lookup).
 * Does NOT require a did and does NOT perform a device DB lookup.
 * Callers should check resp.ok: false means the request is denied.
 * @param {any} env - Worker environment
 * @param {Request} req - The incoming request containing the cid query parameter
 * @returns {Promise<Response>} 204 if authorized, error Response otherwise.
 */
export async function authorizeClient(env, req) {
  const cid = cidOf(req);
  const test = isTest(req);
  ...
  if (cachedCidSig === signotok) {
    log.w("authorizeClient: cid sig cached invalid", cid);
    if (allowLegacyCid && (await isLegacyCidInDb(env, cid))) {
      log.d("authorizeClient: (legacy) ok", cid);
      return r204();
    }
    if (allowAuthorizationBypassForTest && test) {
      log.w("authorizeClient: TEST bypass cid sig invalid");
      return r204();
    }
    return r401("missing/invalid client id");
  }

  if (cachedCidSig !== sigok) {
    // cache miss: derive key and verify
    let k;
    try {
      k = await hmacclientkey(env, rand16, test);
      if (!k) return r500("k unavailable");
    } catch (e) {
      log.e("authorizeClient: key error:", e);
      return r500(`key error: ${e.message}`);
    }

    try {
      const cidsigbuf = await hmacsign(k, cat(rand16, clientmsg));
      const cidsig16expected = new Uint8Array(cidsigbuf).slice(0, 16);
      const valid = safeEq(cidsig16claimed, cidsig16expected);
      sigcache.put(cidkey, valid ? sigok : signotok);
      if (!valid) {
        log.w("authorizeClient: cid invalid", cid);
        if (allowLegacyCid && (await isLegacyCidInDb(env, cid))) {
          log.d("authorizeClient: (legacy) ok", cid);
          return r204();
        }
        if (allowAuthorizationBypassForTest && test) {
          log.w("authorizeClient: TEST bypass cid invalid");
          return r204();
        }
        return r401("missing/invalid client id");
      }
    } catch (e) {
      log.e("authorizeClient: sig error:", e);
      return r500(`${e.message}`);
    }
  }

  log.d("authorizeClient: ok", cid, "t?", test);
  return r204();
}

/**
 * Verifies that the given cid was generated by us (via the HMAC flow or legacy DB lookup).
 * Does not require a did and does not perform a device DB lookup.
 * @param {any} env - Worker environment
 * @param {string} cid - Client identifier (hex string)
 * @param {boolean} test - Whether this is a test-mode request
 * @returns {Promise<boolean>} true if the cid is valid, false otherwise.
 * @throws {Error} if there is an unexpected error during validation.
 */
export async function isCidValid(env, cid, test) {
  const ok = true;
  if (!cid || cid.length < mincidlength || !/^[a-fA-F0-9]+$/.test(cid)) {
    return !ok;
  }
  ...
  if (cachedCidSig === signotok) {
    log.w("isCidValid: cid sig cached invalid", cid);
    if (allowLegacyCid && (await isLegacyCidInDb(env, cid))) {
      log.d("isCidValid: (legacy) ok", cid);
      return ok;
    }
    return !ok;
  }
  ...
  // cache miss: derive key and verify sig
  let k;
  try {
    k = await hmacclientkey(env, rand16, test);
    if (!k) {
      log.e("isCidValid: key unavailable");
      throw new Error("k missing");
    }
  } catch (e) {
    log.e("isCidValid: key error:", e);
    throw e;
  }
  ...
  try {
    ...
    sigcache.put(cidkey, valid ? sigok : signotok);
    if (!valid) {
      log.w("isCidValid: cid invalid", cid);
      if (allowLegacyCid && (await isLegacyCidInDb(env, cid))) {
        log.d("isCidValid: (legacy) ok", cid);
        return ok;
      } else {
